...
make_facet_lc_plot <- function(light_data, lc_model_fitted_data, lc_model_params) {
  ...
  ggplot(mapping = aes(x = par)) +
    ...
    labs(
      y = "ETR (µmol m<sup>-2</sup> s<sup>-1</sup>)", 
...
```{r paper-info, results = "asis", eval = params$output_type == "ms", purl = FALSE}
# Only print journal name in SI for submission
cat("
...
**`r s_figure("dt-control")`**. Recovery of controls (samples kept on moist tissues) during desiccation tolerance test of filmy fern sporophytes from Moorea, French Polynesia. 

**`r s_figure("water-all")`**. Relative water content (RWC) of sporophytes in desiccated state by duration of desiccation treatment (2 or 15 days).
...
ggtree(phy) %<+% filmy_habit %<+% bs_tibble +
  geom_text(aes(label=bootstrap), hjust=-.25, size = 2, color = "grey10") +
  ...
  theme(
    plot.margin = margin(t = 0.1, r = 2.2, b = 0.1, l = 0.5, unit = "in"),
    legend.position = "bottom"
  )
```

**`r s_figure("tree")`**. Maximum likelihood ultrametric phylogenetic tree including all filmy fern species from Moorea, French Polynesia.
Tree obtained by extracting Hymenophyllaceae from the tree of Nitta et al. (2017) with the ‘extract.clade’ function of the ‘ape’ package (Paradis et al. 2004).
...
licl_temp_plot_2012 + licl_temp_plot_2013 + licl_temp_plot_2014 +
  ...
  theme(plot.title = element_markdown(), panel.grid.minor = element_blank())
```

```{r sporophyte-dt-MgNO3, fig.height = 6.6, fig.width = 9, fig.cap = ""}
# Get mean times of each cluster of DT test samples for the selected drying salt
...
mgno3_temp_plot_2012 <- plot_chamber_temp(mgno3_chamber_data_2012, mgno3_time_range_2012) +
  scale_y_continuous(limits = mgno3_temp_range, label = label_number(suffix = "\u00b0C", accuracy = 1)) +
  labs(title = "2012") +
  theme(plot.title = element_text(hjust = 0.5, size = 11)) +
  blank_x_theme()
...
```{r sporophyte-dt-NaCl, fig.height = 6.6, fig.width = 9, fig.cap = ""}
...
# Subset DT chamber data
# the datalogger in use changed part way through.
# 10342564 was used in a combined chamber including both sporos and gametos
chamber_gameto_2013_part_1 <- filmy_dt_chamber %>% filter(serial_no == "10342564", year == "2013")
chamber_gameto_2013_part_2 <- filmy_dt_chamber %>% filter(serial_no == "10140659", year == "2013", generation == "gametophyte")

# chamber_gameto_2013 <- filmy_dt_chamber %>% filter(generation == "gametophyte", year == "2013")
chamber_gameto_2013 <- bind_rows(chamber_gameto_2013_part_1, chamber_gameto_2013_part_2)
...
chamber_gameto_2012_temp_plot + chamber_gameto_2013_temp_plot +
  chamber_gameto_2012_rh_plot + chamber_gameto_2013_rh_plot + 
  dt_times_gameto_2012_plot + dt_times_gameto_2013_plot + 
...
ggplot(plot_data, aes(y = species)) +
  ...
  geom_point(
    ...
    shape = 23, fill = "grey10") +
  scale_color_manual(values = habit_colors, labels = pretty_text) +
  scale_y_discrete(labels = str_abbrev_sp_pretty) +
...
dt_control_plot_data <-
  ...
  filter(!is.na(yield_pre), yield_pre > 0.4) %>%
...
**`r s_figure("water-all")`**. Relative water content (RWC) of sporophytes in desiccated state (after desiccation but before recovery) by duration of desiccation treatment (2 or 15 days). RWC was not measured for all combinations of species and desiccation treatments; missing points indicate data not collected (not RWC of zero).
...
**`r s_figure("water-s")`**. Relative water content (RWC) of sporophytes during desiccation tolerance (DT) test. RWC was not measured for all combinations of species and desiccation treatments; missing points indicate data not collected (not RWC of zero). x-axis indicates condition of sample during DT test; times refer to amount of elapsed time since re-wetting.
...
# Remove one sample that had only outlier points and no good measurements
light_data_all_mod <-
  light_data_all %>%
  group_by(light_id) %>%
  mutate(all_outliers = all(outlier)) %>%
  ungroup() %>%
  filter(!all_outliers) %>%
  select(-all_outliers)
...
lc_plots_sporo_lab <- make_facet_lc_plot(
  light_data = light_data_all_mod %>% filter(generation == "sporophyte", condition == "lab"),
...
lc_plots_sporo_field <- make_facet_lc_plot(
  light_data = light_data_all_mod %>% filter(generation == "sporophyte", condition == "field"),
...
lc_plots_gameto <- make_facet_lc_plot(
  light_data = light_data_all_mod %>% filter(generation == "gametophyte"),
