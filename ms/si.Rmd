...
range_comparison %>%
  ...
  mutate(species = factor(species) %>% fct_reorder(sporo_min_range)) %>%
  ggplot(aes(y = species)) +
  geom_linerange(aes(xmin = gameto_min_range, xmax = gameto_max_range)) +
  geom_linerange(aes(xmin = sporo_min_range, xmax = sporo_max_range, color = habit), size = 3) +
  geom_point(
    data = filter(range_comparison, gameto_min_range < sporo_min_range), 
    aes(x = gameto_min_range)) +
  geom_point(
    data = filter(range_comparison, gameto_max_range > sporo_max_range),
    aes(x = gameto_max_range)) +
  scale_color_manual(values = habit_colors, labels = pretty_text) +
...
**`r s_figure("range")`**. Elevational range of sporophytes and gametophytes of filmy ferns on Moorea, French Polynesia. Observed elevational range width of sporophytes shown with colored bars; range of gametophytes occuring beyond sporophytes shown with lines and dots. Color indicates growth habit.
