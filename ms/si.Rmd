...
**Content:** \figs `r s_figure_num("tree")`--`r s_figure_num("light-curves")`

**`r s_figure("tree")`**. Maximum likelihood ultrametric phylogenetic tree including all Moorean filmy fern species.

**`r s_figure("chamber-s")`**. Conditions inside desiccation chamber and duration of desiccation treatment for sporophytes.

**`r s_figure("chamber-g")`**. Conditions inside desiccation chamber and duration of desiccation treatment for gametophytes.
...
```{r sporophyte-dt-LiCl, fig.height = 7, fig.width = 9.5, fig.cap = ""}
...
licl_time_range_2012 <- c(min = min(licl_chamber_data_2012$date_time), max = max(licl_chamber_data_2012$date_time))
...
# Combine plots
licl_temp_plot_2014 + licl_temp_plot_2013 + licl_temp_plot_2012 +
  licl_rh_plot_2014 + licl_rh_plot_2013 + licl_rh_plot_2012 +
  licl_time_plot_2014 + licl_time_plot_2013 + plot_spacer() +
  plot_layout(ncol = 3, nrow = 3, guides = "collect") +
  plot_annotation(title = "a") &
  theme(plot.tag = element_text(face = "bold"))
```

```{r sporophyte-dt-MgNO3, fig.height = 7, fig.width = 9.5, fig.cap = ""}
# Get mean times of each cluster of DT test samples for the selected drying salt
...
mgno3_time_range_2012 <- c(min = min(mgno3_chamber_data_2012$date_time), max = max(mgno3_chamber_data_2012$date_time))
...
# Combine plots
mgno3_temp_plot_2014 + mgno3_temp_plot_2013 + mgno3_temp_plot_2012 +
  mgno3_rh_plot_2014 + mgno3_rh_plot_2013 + mgno3_rh_plot_2012 +
  mgno3_time_plot_2014 + mgno3_time_plot_2013 + plot_spacer() +
  plot_layout(ncol = 3, nrow = 3, guides = "collect") +
  plot_annotation(title = "b") &
  theme(plot.tag = element_text(face = "bold"))
```

```{r sporophyte-dt-NaCl, fig.height = 7, fig.width = 9.5, fig.cap = ""}
...
# make levels of `serial_no` and `species` the same across datasets
nacl_all_sn <- filmy_dt_chamber %>% 
  filter(salt == "NaCl") %>% 
  pull(serial_no) %>% 
  unique %>% 
  sort %>%
...
nacl_all_species <- sporo_times %>% 
  filter(salt == "NaCl") %>% 
  pull(species) %>% 
  unique %>% 
  sort

nacl_clusters_2013 <- mutate(
  nacl_clusters_2013, 
  # sort SN in numeric order
  serial_no = fct_expand(serial_no, nacl_all_sn) %>% fct_inseq,
  # sort species in reverse-alphabetical order (so 'A' is on top in plot)
  species = fct_expand(species, nacl_all_species) %>% fct_relevel(function (x) sort(x, decreasing = TRUE))
)

nacl_clusters_2014 <- mutate(
  nacl_clusters_2014, 
  serial_no = fct_expand(serial_no, nacl_all_sn) %>% fct_inseq,
  species = fct_expand(species, nacl_all_species) %>% fct_relevel(function (x) sort(x, decreasing = TRUE))
)
...
nacl_chamber_data_2012 <- 
  filmy_dt_chamber %>% 
  filter(salt == "NaCl", date_time > "2012-07-31 18:10:00 UTC", date_time < "2013-01-01 00:10:00 UTC")

nacl_time_range_2012 <- c(min = min(nacl_chamber_data_2012$date_time), max = max(nacl_chamber_data_2012$date_time))
...
nacl_temp_range <- filter(filmy_dt_chamber, salt == "NaCl") %>%
  summarize(min = min(temp), max = max(temp)) %>%
  pivot_longer(everything()) %>%
  arrange(value) %>%
  pull(value)

nacl_rh_range <- filter(filmy_dt_chamber, salt == "NaCl") %>%
  mutate(rh = rh * 0.01) %>%
...
serial_no_cols <- c(
  brewer_pal(palette = "Dark2")(5),
  "grey20") %>%
  set_names(
    bind_rows(nacl_chamber_data_2013, nacl_chamber_data_2014) %>%
      filter(serial_no != "missing") %>%
      pull(serial_no) %>%
      unique %>%
      sort %>%
      c("missing")
...
nacl_time_plot_2013 <- 
  ...
  geom_linerange(position = position_dodge(0.5)) +
  geom_point(position = position_dodge(0.5)) +
  scale_y_datetime(limits = nacl_time_range_2013) +
...
nacl_time_plot_2014 <- 
  ggplot(
    nacl_clusters_2014, 
    aes(x = species, ymin = pre, ymax = post, y = post, 
        group = species_duration, color = serial_no)
  ) +
  geom_linerange(position = position_dodge(0.5)) +
  geom_point(position = position_dodge(0.5)) +
  scale_y_datetime(limits = nacl_time_range_2014) +
  scale_x_discrete(labels = str_abbrev_sp_pretty, drop = FALSE) +
...
nacl_rh_plot_2012 <- 
  ggplot(
    nacl_chamber_data_2012, 
    aes(x = date_time, y = 0.01*rh)
  ) +
  geom_line(size = 0.25) +
  scale_x_datetime(limits = nacl_time_range_2012) +
  scale_y_continuous(
    label = scales::label_percent(accuracy = 1), 
    limits = nacl_rh_range) +
  labs(
    y = "Relative humidity",
    color = "Data logger\nserial no.") +
  guides(color = "none") +
...
nacl_rh_plot_2013 <- 
  ...
  scale_color_manual(values = serial_no_cols) +
  geom_line(size = 0.25) +
  scale_x_datetime(limits = nacl_time_range_2013) +
  scale_y_continuous(
    label = scales::label_percent(accuracy = 1), 
    limits = nacl_rh_range) +
  labs(
...
nacl_rh_plot_2014 <- 
  ...
  scale_color_manual(values = serial_no_cols) +
  geom_line(size = 0.25) +
  scale_x_datetime(limits = nacl_time_range_2014) +
  scale_y_continuous(
    label = scales::label_percent(accuracy = 1), 
    limits = nacl_rh_range) +
  labs(
...
nacl_temp_plot_2014 + nacl_temp_plot_2013 + nacl_temp_plot_2012 +
  nacl_rh_plot_2014 + nacl_rh_plot_2013 + nacl_rh_plot_2012 +
  nacl_time_plot_2014 + nacl_time_plot_2013 + plot_spacer() +
  plot_layout(ncol = 3, nrow = 3, guides = "collect") +
...
**`r s_figure("chamber-s")`**. Temperature, relative humidity (RH), and duration of desiccation treatment for sporophytes. Relative humidity was controlled by placing drying salts inside desiccation chambers: **a**, LiCl (-282 MPa, 18 % RH); **b**, Mg(NO~3~)~2~ (-86 MPa, 58 % RH); **c**, NaCl (-38 MPa, 80 % RH; **c**. Temperature and RH measured with Hobo ProV2 dataloggers (Onset Computer Corporation, Bourne, Massachusetts, USA) in 2013 and 2014 and **--FIXME: add datalogger model--** dataloggers in 2012. For bottom-most panels (duration of desiccation treatment), line indicates duration and point indicates removal of sample from chamber. For **c**, multiple chambers were set up and tracked with different data loggers; serial number of data logger indicated by color. "missing" indicates data were not collected on the correspondence between samples and data loggers (chambers).
