...
Desiccation tolerance tended to increase with VPD for sporophytes, but not gametophytes (`r figure("corr")`).
These trends were marginally non-significant when analyzed using PGLS for sporophytes (*P* = `r sporo_vpd_pval`) and not significant for gametophytes (*P* = `r gameto_vpd_pval`; `r table("pgls")`).

No significant relationship was detected between range size of gametophytes and DT (`r figure("corr")`, `r table("pgls")`).
...
phylosig %>%
  ...
  select(trait, kval, lambda) %>%
  kbl(format = "latex", booktabs = TRUE, col.names = c("", "\\textit{K}", "λ"), escape = FALSE) %>%
  pack_rows(
    "Sporophytes", 
...
glmm_summary %>%
  ...
  select(fixed_effects, DIC, delta_DIC) %>%
  kbl(format = "latex", booktabs = TRUE, col.names = c("Fixed effects", "DIC", "delta DIC"), escape = TRUE) %>%
...
**`r table("pgls")`** Phylogenetic generalized least squares (PLGS) models of desiccation tolerance as a function of environmental water potential (VPD) or the range of gametophytes beyond sporophytes (G beyond S) in filmy ferns from Moorea, French Polynesia
