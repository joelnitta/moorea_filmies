...
**`r figure("sporo-dt")`** Results of sporophyte desiccation tolerance test on filmy fern sporophytes from Moorea, French Polynesia.
...
```{r sporo-dt, fig.width = 6.85, fig.height = 7.5, fig.cap = ""}
...
### Make plot ###
ggplot(data = plot_data, aes(x = rec_time, y = recovery, group = interaction(dry_time, salt), shape = salt, fill = salt)) +
  ...
  scale_fill_manual(name = "Desiccation Intensity",
                    ...
                    breaks = c("NaCl", "MgNO3",  "LiCl", "Control"),
                    labels = c("-38 MPa", "-86 MPa", "-282 Mpa", "Control")) +
  scale_y_continuous(label = label_percent(accuracy = 1), limits = c(-0.1,1.1), expand = c(0,0), breaks=c(0,.2,.4,.6,.8,1.0)) +
...
ggsave(here::here("results/ms/fig_1_sporo_dt.eps"), width = 6.85, height = 7.5)
```
