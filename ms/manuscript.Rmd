...
A total of `r n_filmy_sp` filmy fern species are known from Moorea, including multiple terrestrial, saxicolous, and epiphytic species [@Murdock2003; @Ranker2005; @Nitta2011b; @Nitta2017; @Florence] (`r figure("filmy-habit")`).
...
In total, `r n_distinct(range_types$species)` filmy fern taxa (including two species that were recognized as multiple varieties) were observed on Moorea.
...
In several species, gametophytes exceeded the maximum or minimum elevational range of sporophytes (Table 1) including *Callistopteris apiifolia* (C. Presl) Copel., *Crepidomanes minutum* var. 2 and 3, *Hymenophyllum digitatum* (Sw.) Fosberg, *Hymenophyllum javanicum* Spreng., *Hymenophyllum pallidum* (Blume) Ebihara & K.
...
Mean daily temperature was strongly correlated with elevation, and dropped ca.
...
A general correlation of DT with habitat was observed in sporophytes (`r figure("filmy-dt")`).
Both terrestrial species (*Abrodictyum dentatum* (Bosch) Ebihara & K Iwats. and *C. apiifolia*) lacked even moderate DT, and failed to recover from the gentlest treatment of 2 d at --38 MPa (more stringent tests were not conducted on these species after confirming their lack of DT at this level).
...
Gametophytes of low elevation epiphytes had the highest DT (\circa{ }80% recovery), followed by high elevation epiphytes and saxicolous species (both with recovery from \circa{ }25% to 75%), with terrestrial species essentially non-tolerant (less than 20% recovery; Fig. 5).
...
**`r figure("filmy-habit")`** Examples of filmy ferns from French Polynesia.
...
**`r figure("filmy-dt")`** Results of desiccation tolerance test on filmy fern sporophytes (**a**) and gametophytes (**b**) from Moorea, French Polynesia.
...
default_theme <- theme_set(theme_bw(base_size = 10))
