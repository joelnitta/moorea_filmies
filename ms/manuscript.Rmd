...
For all further analyses, we used methods that account for phylogenetic history for physiological traits that showed significant phylogenetic signal in at least one of the two metrics, and standard methods otherwise.
...
Results of the tests for phylogenetic signal (Blomberg's *K* and Pagel's λ*)* were mostly similar for sporophytes and gametophytes, revealing significant phylogenetic signal for DT when measured with *K* (gametophytes *K* = `r gameto_recover_k`, *P* = `r gameto_recover_k_pval`; sporophytes *K* = `r sporo_recover_k`, *P* = `r sporo_recover_k_pval`), but no significant signal in either metric for PPFD~95%~ or ETR~max~ (`r table("physig")`). Results for phylogenetic signal in DT as measured with λ differed slightly; although gametophytes showed significant signal (λ = `r gameto_recover_lambda`, *P* = `r gameto_recover_lambda_pval`) sporophytes did not (λ =`r sporo_recover_lambda`, *P* = `r sporo_recover_lambda_pval`).
