...
n_dt_species_gameto <- recovery_species_means %>% 
...
n_dt_species_both <- recovery_species_means %>% 
  count(species, generation) %>% 
  group_by(species) %>% 
...
# Vector of species that I know *after* running the t-test have significant
# differences and discuss in the MS.
species_with_signif_dt_diff <- c("Hymenophyllum_polyanthos", "Hymenophyllum_digitatum", "Crepidomanes_minutum2", "Polyphlebium_endlicherianum", "Polyphlebium_borbonicum")

t_test_results %>% 
  filter(response == "recover", p_value < 0.05) %>%
  # Make sure that species with signif diff in DT mentioned
  # in MS match those in the actual analysis results
  verify(setequal(.$species, species_with_signif_dt_diff)) %>%
  select(species, estimate, p_value) %>%
  arrange(p_value)

```


Ability to recover from desiccation was tested in sporophytes of `r n_dt_species_gameto` species and gametophytes of `r n_dt_species_gameto` species, including `r n_dt_species_both` species with both life phases (Appendix S3).
...
Light responses were measured in sporophytes of 18 species and gametophytes of 11 species, including nine species with both life phases (Appendix S4).
...
### Environmental water potential and DT
