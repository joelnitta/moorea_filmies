...
During this study, `r n_distinct(moorea_sites$site)` sampling sites were established from \circa{ }200 m to 1200 m primarily along trails to the summits of three large peaks on Moorea: Mt. Rotui (899 m), Mt. Mouaputa (880 m), and Mt. Tohiea (1206 m; Fig. 2).
...
Unlike many other fern species, filmy fern gametophytes have unique morphologies that allow them to be identified to family [\eg, filamentous, ribbon with round gemmae; @Nayar1971].
...
For sporophytes, 8--16 individuals (one individual = single whole frond including a \circa{ }2 cm section of rhizome) were used per treatment, and all individuals for each species came from a single population.
...
Ability to recover from desiccation was quantified by comparing pre- and post-treatment chlorophyll fluorescence yield [@WatkinsJr.2007c].
...
```{r phy-sig-stats}
sporo_recover_k <- phylosig %>% filter(generation == "sporo", trait == "recover_mean") %>% pull(kval) %>% jntools::round_t(2)
sporo_recover_k_pval <- phylosig %>% filter(generation == "sporo", trait == "recover_mean") %>% pull(k_pval) %>% jntools::round_t(3)
sporo_recover_lambda <- phylosig %>% filter(generation == "sporo", trait == "recover_mean") %>% pull(lambda) %>% jntools::round_t(2)
sporo_recover_lambda_pval <- phylosig %>% filter(generation == "sporo", trait == "recover_mean") %>% pull(lambda_pval) %>% jntools::round_t(3)

# Make sure that all ETRmax and PAR have pval > 0.05
phylosig %>% 
  filter(trait != "recover_mean") %>%
...
DT of gametophytes did not significantly exceed that of sporophytes for any species except for saxicolous *P. endlicherianum* (Fig. 5; *t*-test, *P* = `r ttest_pval("Polyphlebium_endlicherianum", "recover")`), and was significantly lower in gametophytes than sporophytes of some high elevation epiphytic species including *C. minutum* var. 2, *H.* *digitatum*, *H. polyanthos*, and *P. borbonicum* (Fig. 5; *t*-test, *P* = `r ttest_pval("Crepidomanes_minutum2", "recover")`, `r ttest_pval("Hymenophyllum_digitatum", "recover")`, `r ttest_pval("Hymenophyllum_polyanthos", "recover")`, and `r ttest_pval("Polyphlebium_borbonicum", "recover")`, respectively).
...
mean_par_sporo <- light_overall_means %>% filter(generation == "sporo") %>% pull(par_pretty)
mean_par_gameto <- light_overall_means %>% filter(generation == "gameto") %>% pull(par_pretty)
mean_etr_sporo <- light_overall_means %>% filter(generation == "sporo") %>% pull(etr_pretty)
mean_etr_gameto <- light_overall_means %>% filter(generation == "gameto") %>% pull(etr_pretty)
```
