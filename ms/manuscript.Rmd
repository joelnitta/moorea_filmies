...
A total of `r n_filmy_sp` filmy fern species are known from Moorea, including multiple terrestrial, saxicolous, and epiphytic species [@Murdock2003; @Ranker2005; @Nitta2011b; @Nitta2017; @Florence] (\figs `r figure_num("habit")`, `r s_figure_num("tree")`; all supplemental figures available in Online Resource 1).
...
Conditions inside the desiccation chambers were monitored during the experiment using Track-It RH/Temp dataloggers (Monarch Instrument, Amherst, NH) logging every 10 min (2012 field season) or Hobo ProV2 dataloggers logging every 5 min (2013, 2014 field seasons).
The desiccation chambers were kept in an air-conditioned room. 
Temperature inside the chambers was mean `mean(filmy_dt_chamber$temp) %>% round(1)` ± SD `r sd(filmy_dt_chamber$temp) %>% round(2)` °C (\figs `r s_figure_num("chamber")`).
...
glmm_params %>% 
  filter(response == "recovery", pMCMC < 0.05) %>%
...
Relative humidity in the growth chambers was high when samples were first inserted, then gradually decreased to the target value maintained by each drying salt (\figs{ }`r s_figure_num("chamber")`).
