...
4.) Gametophytes with wider ranges have higher DT than those with with narrower ranges.
...
Salts used for desiccation and their corresponding mean water potentials and approximate relative humidity are as follows: LiCl (-282 MPa, 18 % RH), Mg(NO~3~)~2~ (-86 MPa, 58 % RH), and NaCl (-38 MPa, 80 % RH).
...
Results of the tests for phylogenetic signal (Blomberg's *K* and Pagel's λ*)* were similar for sporophytes and gametophytes, revealing significant phylogenetic signal for DT (gametophytes *K* = `r gameto_recover_k`, *P* = `r gameto_recover_k_pval` and λ = `r gameto_recover_lambda`, *P* = `r gameto_recover_lambda_pval`; sporophytes *K* = `r sporo_recover_k`, *P* = `r sporo_recover_k_pval` and λ = `r sporo_recover_lambda`, *P* = `r sporo_recover_lambda_pval`), but no significant signal for PPFD~95%~ or ETR~max~ (`r table("physig")`).
...
DT of gametophytes did not significantly exceed that of sporophytes for any species, and was significantly lower in `r dt_g_less_than_s` (`r figure("combined")`; *t*-test, *P* = `r dt_g_less_than_s_pvals`, respectively).
...
Relative humidity in the growth chambers was high when samples were first inserted, then gradually decreased to the target value maintained by each drying salt (\figs{ }`r s_figure_num("chamber-s")`, `r s_figure_num("chamber-g")`).
...
The GLMM including the effect of generation only was selected as the best-fitting model for DT (`r table("glmm")`). 
...
Photosynthetic optima tended to vary between life phases, but not across growth habits or range type: sporophytes were adapted for higher light levels, whereas gametophytes were adapted for lower light levels (\figs `r figure_num("combined")`, `r s_figure_num("light-curves")`).
...
The GLMM including the effect of generation only was selected as the best-fitting model for ETR~max~, and growth habit + generation was selected for PPFD~95%~ (`r table("glmm")`).
However, in the GLMM for PPFD~95%~, only the generation parameter was significant (`r table("glmm-pars")`).
...
This trend was marginally non-significant when analyzed using PGLS (*P* = `r sporo_vpd_pval`; `r table("pgls")`).
...
Since controls kept at 100% RH maintained F~v~/F~m~ values near pre-treatment levels in all sporophytes (`r s_figure("dt-control")`), it is unlikely that the lack of tolerance seen in terrestrial species is due solely to removal from the soil.
...
However, we found that gametophytes had lower DT than sporophytes regardless of growth habit (`r figure("compare")`).
...
Although we predicted that species with gametophytes that occur far beyond the range of their sporophytes would have higher DT, this was not the case.
...
Photosynthetic parameters did not vary significantly between species from different habitats.
