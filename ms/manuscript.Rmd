...
rel_water_species_means %>%
  ...
  mutate(sum_val = glue("{mean} ± {sd} (*n* = {n})"))

rel_water_table %>% filter(salt == "LiCl", dry_time == "15") %>% pull(sum_val)
```
...
Relative water content in the desiccated state ranged from `r rel_water_table %>% filter(salt == "LiCl", dry_time == "15", rec_time == "desiccated") %>% pull(sum_val)` (--282 MPa for 15 d) to `r rel_water_table %>% filter(salt == "NaCl", dry_time == "2", rec_time == "desiccated") %>% pull(sum_val)` (--38 MPa for 2 d), and returned to \circa{ }100 % upon re-wetting in all species sampled (`r s_figure("water-s")`).
