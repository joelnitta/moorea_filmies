...
In extant vascular plants (tracheophytes), DT occurs in certain non-vegetative stages (\eg, seeds and pollen), but has largely been lost in vegetative tissues (with some notable exceptions, the so-called 'resurrection plants').
...
1.) Epiphytic filmy ferns have higher DT and photosynthetic optima than terrestrial species.
2.) Degree of DT is correlated with environmental water availability.
3.) Gametophytes with wider ranges have higher DT than those with with narrower ranges.
...
A total of `r n_filmy_sp` filmy fern species are known from Moorea, including multiple terrestrial, saxicolous, and epiphytic species [@Murdock2003; @Ranker2005; @Nitta2011b; @Nitta2017; @Florence] (Figs. `r figure_num("habit")`, `r s_figure_num("tree")`).
...
Therefore, we used DNA barcoding to identify filmy fern gametophytes to species as described in @Nitta2017.
...
This dataset includes relative humidity (RH) and temperature measured every 15 min from `r climate_start` to `r climate_end` at the sampling sites with Hobo ProV2 dataloggers (Onset Computer Corporation, Bourne, Massachusetts, USA) and vapor pressure deficit (VPD) calculated from these two values.
...
The desiccation chambers were kept in an air-conditioned room, and temperature inside the chambers ranged from 22 °C to 25 °C (Figs. `r s_figure_num("chamber-s")`, `r s_figure_num("chamber-g")`).
...
Following the desiccation treatment, plants were rewetted by wrapping them in wet tissues using water from a local stream.
...
We measured relative water content (RWC) in a subset of samples by recording the mass of samples prior to the DT test (fresh mass), at each step of the DT test (turgid mass), then after drying them overnight in a drying oven at 65 °C following the DT test (dry mass). 
...
We therefore applied the same treatment for all gametophytes (2 d at -86 MPa), and verified species of each individual after the experiment using the DNA barcode approach described above.
...
This tree is in accord with previously published, more densely sampled phylogenies [@Pryer2001a; @Dubuisson2003; @Hennequin2006; @Ebihara2007a].

We quantified the degree of phylogenetic signal in physiological traits using Blomberg's *K* [@Blomberg2003] and Pagel's λ [@Pagel1999; @Freckleton2002] as implemented with the \function{phylosig} function in the \package{phytools} package [@Revell2012].
...
We tested the significance of *K* by comparing the observed value against values from a null distribution of \num{1000} phylogenies with the traits randomly shuffled across the tips.
...
Since DT showed significant phylogenetic signal (see Results), we used phylogenetic generalized least squares (PGLS) [@Freckleton2002] to test for a relationship between environmental water potential and DT while taking phylogeny into account, using species' means.

To test whether gametophytes with wider ranges have higher DT, we calculated the extent of gametophyte range beyond sporophytes ("G beyond S") as the total elevational range (m) gametophytes were observed growing beyond sporophytes at either end of the elevational gradient. 
...
Datasets used in this study are available on FigShare (\#\#\#\#\#\#). 
Code used to analyze the data and generate figures and this manuscript are available on GitHub (https://github.com/joelnitta/moorea-filmies).
...
Both terrestrial species (*Abrodictyum dentatum* (Bosch) Ebihara & K Iwats. and *C. apiifolia*) lacked even moderate DT, and failed to recover from the gentlest treatment of 2 d at --38 NaCl (more stringent tests were not conducted on *A. dentatum* after confirming its lack of DT at this level).
...
Control samples generally did not show any decrease in F~v~/F~m~ throughout the duration of the experiment (`r s_figure("dt-control")`). 
Relative water content reached \circa{ }0 % in the desiccated state (`r s_figure("water-all")`), and returned to \circa{ }100 % upon re-wetting in all species sampled (`r s_figure("water-s")`).
...
Relative humidity in the growth chambers was high when samples were first inserted, then decreased to the target value maintained by each drying salt (Figs. `r s_figure_num("chamber-s")`, `r s_figure_num("chamber-g")`).
Brief fluctuations to higher humidities were observed when chambers were opened to insert or remove samples (`r s_figure("chamber-s")`).

DT of gametophytes did not significantly exceed that of sporophytes for any species except for saxicolous *P. endlicherianum* (Fig. 5; *t*-test, *P* = ), and was significantly lower in gametophytes than sporophytes of some high elevation epiphytic species including *C. minutum* var. 2, *H.* *digitatum*, *H. polyanthos*, and *P. borbonicum* (`r figure("combined")`; *t*-test, *P* = XXX, XXX, XXX, and XXX, respectively).
...
Comparison of DT with the minimum water potential envelope of each species (`r figure("corr")`) indicates that sporophytes of non-tolerant terrestrial and saxicolous species may not be able to occur in habitats with minimum water potential exceeding \circa{ }-7 MPa.
...
However, we found that within high elevation epiphytes, gametophytes had lower DT than sporophytes, and only one species (saxicolous *P. endlicherianum*) with gametophytes that had significantly greater DT than sporophytes (`r figure("compare")`).
...
Rather, we observed a clear intergenerational difference, with gametophytes consistently adapted for lower light levels than sporophytes (`r figure("combined")`, `r table("glmm")`).
...
We detected significant phylogenetic signal in DT, suggesting that phylogeny should be considered when interpreting differences in DT between species (`r table("physig")`).
...
```{r filmies-fig, fig.width = 6.85, results = "asis", purl = FALSE}
...
```{r filmy-dt-plot, fig.width = 6.85, fig.height = 7.5, fig.cap = "", purl = FALSE}
dt_plot_data %>%
...
```{r gameto-dt-plot, fig.width = 6.85, fig.height = 7.5, fig.cap = "", purl = FALSE}
...
```{r combined-plot, fig.width = 6.85, fig.height = 5, fig.cap = "", purl = FALSE}
...
```{r corr-plot, fig.width = 6.85, fig.height = 5, fig.cap = "", purl = FALSE}
